...
import { Share2, ImageDown, Link2 } from "lucide-react";
    ...
    <div className="fixed bottom-6 right-6 z-50" style={{ bottom: "max(1.5rem, calc(env(safe-area-inset-bottom) + 1rem))" }}>
      {menuOpen && (
        <>
          ...
          <div className="absolute bottom-14 right-0 mb-1 flex flex-col gap-2 items-end">
            <Button
              className="rounded-full shadow-md h-10 px-4 gap-2 font-medium"
              variant="secondary"
              onClick={handleShareImage}
              style={{ touchAction: "manipulation" }}
            >
              <ImageDown className="h-4 w-4 shrink-0" />
              <span className="pointer-events-none">Share image</span>
            </Button>
            <Button
              className="rounded-full shadow-md h-10 px-4 gap-2 font-medium"
              variant="secondary"
            ...
            >
              ...
              <span className="pointer-events-none">Share link</span>
            </Button>
          </div>
        </>
      )}
